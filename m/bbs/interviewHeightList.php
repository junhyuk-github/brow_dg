...
?>
					...
					<li>
						<div class="itv-hd">
							<? if ( $iFile ) { ?>
							<div class="img-auto"><img src="<?=$fullUpImg?>" alt=""></div>
							<? } ?>
							<div class="txt-box">
								<span class="cn">[<?=$gubunTxt1?> <?=$gubunTxt2?>]</span>
								<span class="sn"><strong><?=$writerName?></strong> <?=$userGroupTxt?></span>
								<a href="./interviewView.php?idx=<?=$idx?>" class="itv-pnt">"<?=$subject?>"</a>
							</div>
						</div>
					</li>
					<?
					...
					?>
					<div class="more-btn-wrap" id="paging1_<?=$_REQUEST['pno']?>">
						<button type="button" class="btn-more" onClick="getContentList('/m/bbs/interviewHeightList.php', 'contentHeightDiv');">수강생 인터뷰 더보기</button>
					</div>
					<?
						}
					}

...
<div id="wrap" class="main pg_detail">
	...
	<div id="" class="board-frame stReview">
		<div class="thumbnail">
			<div class="thumb-title">
				<img src="/common/img/title/review.png" alt="">
			</div>
		</div>
		<div class="pg-tit">
			<h2>수강생 실제후기</h2>
			...
			<div class="minframe">
				<section class="view-reg"> 
					<div class="vw-tit-box">
						...
						<span class="">
							...
							<img class="icon-img" src="/common/img/commu/i-tnfBrow.png" alt="">
                            <?php } ?>
						...
						<!-- 2. 비로그인 상태일때 -->
						<div class="none-id">
							<div class=""><img class="icon-img" src="/common/img/commu/i-tnfBrow.png" alt="">후기 작성은 로그인 후 이용할 수 있습니다.</div>
							<button type="button" onclick="window.location='/m/member/login.php';" class="btn-border">로그인</button>
						</div>
						<? } ?>
					</div>
					...
					<div class="vw-reg-form">
						...
						<div class="insert-box">
							<div class="inner">
								<? if ( $common['userIdx'] ) { ?>
								<textarea name="contents" id="contents" cols="" rows="4" class="form-control" placeholder="후기를 입력해주세요. (2000자 이내)"></textarea>
								<? } else { ?>
								...
								<div class="vw-img-wrap">
									...
									<div class="vw-upload">
										...
										<div class="btn-vwf" onclick="document.all.upImg1.click()">
											<img src="/common/img/commu/i-camera.png" alt="">
										</div>
									</div>
									<? } else { ?>
									<div class="vw-upload">
							...
							<div class="vw-reg-btn">
								<? if ( $common['userIdx'] ) { ?>
								<button type="button" onClick="formReview('', 'N');" class="btn-border">등록</button>
								<? } else { ?>
								<button type="button" onClick="alert('로그인 후 사용해주세요.');" class="btn-border">등록</button>
								<? } ?>
							</div>
						</div>
					</div>
				...
				<section class="view-list-area">
					<div class="vw-list-head">
						<span class="ttl-count">전체 후기 <?=$page['totalCount']?> 건</span>
					</div>
					<div class="vw-sort">
						<button type="button" class="<? if ( $order == '1' ) { echo ' active'; } ?>" onclick="window.location='<?=$selfPage?>?order=1';">최신순</button>
						<button type="button" class="<? if ( $order == '2' ) { echo ' active'; } ?>" onclick="window.location='<?=$selfPage?>?order=2';">오래된순</button>
					</div>
					<ul class="vw-list">
						<?php
						if ( $bbsList ) {
				...
				<div class="pagi-wrap">
					<ul class="pagination">
						...
						?>
							<? if ( $i == $page['pno'] ) { ?>
								<li class="pg-num active"><a class="pn-link" href="#"><?=$i?></a></li>
							<? } else { ?>
								<li class="pg-num"><a class="pn-link" href="<?=$selfPage?>?pno=<?=$i?><?=$qry?>"><?=$i?></a></li>
							<? } ?>
						<?
						}

(() => {
	...
	function checkMenu() {
		const scrollDownElements = document.getElementsByClassName('scroll-down');
	  
		if (yOffset > 10) {
		  document.body.classList.add('local-nav-sticky');
		  
		  // HTMLCollection에서 각 요소에 접근하여 클래스를 추가합니다.
		  Array.from(scrollDownElements).forEach(element => {
			element.classList.add('down');
		  });
		} else {
		  document.body.classList.remove('local-nav-sticky');
		  
		  // HTMLCollection에서 각 요소에 접근하여 클래스를 제거합니다.
		  Array.from(scrollDownElements).forEach(element => {

...
?>
								...
								<div class="add-reply">
									<div class="ar-head">
										...
										<? } ?>
										<span class="right">댓글 <?=$commentCnt?></span>
									</div>
									<div class="vw-comment-reg-box" id="replyRegMain_<?=$bIdx?>">
										<div class="inner">
											<textarea id="comment" name="comment" cols="" rows="4" class="form-control"></textarea>
											<button type="button" class="btn-border" onClick="actionComment(this.form, 'frmComment_<?=$bIdx?>', 'commentDiv<?=$bIdx?>',  '','N');">등&nbsp;&nbsp;록</button>
										</div>
									</div>
								</div>
								</form>
								...
								<ul class="reply-list">
									...
									<li class="reply-item">
										<div class="rply-head">
											<span class="vw-id">
												<span class="icon l<? if ( $cUserGroup == '2' ) { echo '1'; } else if ( $cUserGroup == '3' ) { echo '2'; } else if ( $cUserGroup == '4' ) { echo '3'; } ?>"></span>
										...
										<div class="add-reply">
											...
											<div class="vw-comment-reg-box" id="replyRegSub_<?=$bIdx?>_<?=$cIdx?>">
												<div class="inner">
													<textarea id="comment" name="comment" rows="4" class="form-control"></textarea>
													<button type="button" class="btn-border" onClick="actionComment(this.form, 'frmComment_<?=$bIdx?>_<?=$cIdx?>', 'commentDiv<?=$bIdx?>', '','N');">등&nbsp;&nbsp;록</button>
												</div>
											</div>
										</div>
									</li>
